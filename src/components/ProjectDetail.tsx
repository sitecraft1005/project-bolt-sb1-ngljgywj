  ...
  return (
    <div className="pt-16 sm:pt-20">
      <div className="container mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <button 
          onClick={onBack}
          className="flex items-center text-yellow-600 hover:text-yellow-700 mb-6 sm:mb-8 text-sm sm:text-base"
        >
        ...
        <div className="grid md:grid-cols-2 gap-8 sm:gap-12 mb-12 sm:mb-16">
          <div className="order-2 md:order-1">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-4">{projectData.title}</h1>
            <span className="inline-block bg-yellow-500 text-white px-3 sm:px-4 py-1 rounded-full text-xs sm:text-sm font-medium mb-4">
              {projectData.type}
            </span>
            <p className="text-base sm:text-lg text-gray-600 mb-6">{projectData.fullDescription}</p>
            
            <div className="space-y-3 sm:space-y-4">
              {projectData.results.map((result, index) => (
                <div key={index} className="flex items-center">
                  <Check size={18} className="sm:w-5 sm:h-5 text-yellow-500 mr-2 flex-shrink-0" />
        ...
        <div className="grid md:grid-cols-2 gap-8 sm:gap-12 mb-12 sm:mb-16">
          ...
          <div className="bg-white p-6 sm:p-8 rounded-xl shadow-md">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-6">Technologies Used</h2>
            <div className="flex flex-wrap gap-2 sm:gap-3">
              {projectData.technologies.map((tech, index) => (
                <span 
                  key={index}
                  className="bg-gray-100 text-gray-700 px-3 sm:px-4 py-1 sm:py-2 rounded-full text-xs sm:text-sm"
                >
                  {tech}
                </span>
              ))}
        ...
        <div className="mb-12 sm:mb-16">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-6">Project Gallery</h2>
